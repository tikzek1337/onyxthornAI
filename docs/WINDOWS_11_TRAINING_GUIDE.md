# Windows 11 training guide for onyxthornAI
...
По умолчанию в `configs/onyxthornai_chat_17m.yaml` стоит `max_steps: 4500`.
...
```powershell
python -m onyxliteai.generate --checkpoint runs/onyxthornai_chat_tiny/best.pt --tokenizer tokenizer/onyxthornai_tokenizer.json --prompt "Привет, поболтай со мной" --temperature 0.75 --top_k 50 --top_p 0.95 --max_new_tokens 500
```

## 7. Интерактивный чат

```powershell
python -m onyxliteai.chat --checkpoint runs/onyxthornai_chat_tiny/best.pt --tokenizer tokenizer/onyxthornai_tokenizer.json --temperature 0.75 --top_k 50 --top_p 0.95 --max_new_tokens 600
```
